...
import ProductDeleteSection from '@/pages/Products/Partials/ProductDeleteSection.vue';
...
import { AlertCircle, Package, Pencil, SlidersHorizontal } from 'lucide-vue-next';
...
const handleModalClose = (modal: 'edit' | 'adjust', data?: any, options?: { reload?: string[]; message?: string }) => {
    if (modal === 'edit') showModal.value = false;
    if (modal === 'adjust') showAdjustModal.value = false;

    if (data && options) {
        if (options.reload) router.reload({ only: options.reload });
        if (options.message) {
...
const closeModal = (product?: ProductWithRelations) => {
    handleModalClose('edit', product, { reload: ['product'], message: 'Product updated successfully' });
};
...
const closeAdjustModal = (product?: ProductWithRelations) => {
    handleModalClose('adjust', product, { reload: ['product', 'movements'], message: 'Stock adjusted successfully' });
};
...
<template>
    <AuthenticatedLayout>
        <div class="space-y-6">
            ...
            <ProductFormModal :show="showModal" :product="product" @close="closeModal($event)" />
